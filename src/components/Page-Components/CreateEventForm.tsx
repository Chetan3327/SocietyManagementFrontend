...
const inputClasses =
  "w-full px-3 py-2 mt-2 text-gray-900 bg-gray-200 border border-gray-300 rounded focus:outline-none focus:ring focus:border-blue-300";
...
const CreateEventForm = () => {
  ...
  const onSubmit = async (data: formData) => {
    try {
      ...
      const submissionData = {
        ...data,
        EnrollmentNo: parseInt(data.Enrollment, 10),
  ...
  return (
    <div className="h-full w-full flex items-center justify-center py-10 px-4 bg-gray-200">
      {isSubmitted ? (
        <div className="bg-green-100 p-6 rounded-lg shadow-lg w-full max-w-screen-md flex flex-col items-center">
          <h2 className="text-3xl font-bold text-green-800">Thank you!</h2>
          <p className="text-center text-green-700">You have successfully registered for the event.</p>
        </div>
      ) : (
        <form
          onSubmit={handleSubmit(onSubmit)}
          className="bg-white p-6 rounded-lg shadow-lg w-full max-w-screen-md"
        >
          <h2 className="mb-10 text-3xl font-bold text-center text-gray-900">Register For The Event</h2>

          {/* Full Name */}
          <div className="flex flex-col sm:flex-row sm:items-center mb-4">
            <label className="block text-gray-700 font-bold sm:w-1/3" htmlFor="name">Full Name</label>
            <input
          ...
          {/* Email */}
          <div className="flex flex-col sm:flex-row sm:items-center mb-4">
            <label className="block text-gray-700 font-bold sm:w-1/3" htmlFor="email">Email</label>
            <input
              className={inputClasses}
              type="text"
              id="email"
          ...
          {/* Branch */}
          <div className="flex flex-col sm:flex-row sm:items-center mb-4">
            <label className="block text-gray-700 font-bold sm:w-1/3" htmlFor="branch">Branch</label>
          ...
          {/* Batch */}
          <div className="flex flex-col sm:flex-row sm:items-center mb-4">
            <label className="block text-gray-700 font-bold sm:w-1/3" htmlFor="batch">Batch</label>
            <select {...register("Batch")} className={inputClasses}>
              {Batch.map((batch, index) => (
          ...
          {/* Enrollment No */}
          <div className="flex flex-col sm:flex-row sm:items-center mb-4">
            <label className="block text-gray-700 font-bold sm:w-1/3" htmlFor="enrollment">Enrollment No.</label>
            <input
              className={inputClasses}
              type="text"
              id="enrollment"
          ...
          {errors.Enrollment && <span className="text-red-500">{errors.Enrollment.message}</span>}

          {/* Society */}
          <div className="flex flex-col sm:flex-row sm:items-center mb-4">
            <label className="block text-gray-700 font-bold sm:w-1/3" htmlFor="society">Society</label>
            <select {...register("Society")} className={inputClasses}>
              {societyOptions.map((society) => (
                <option key={society.id} value={society.id}>
                  {society.name}
                </option>
          ...
          {errors.Society && <span className="text-red-500">{errors.Society.message}</span>}

          <button
            type="submit"
            className="w-full px-4 py-2 mt-6 text-xl text-white bg-gray-800 rounded-lg hover:bg-gray-900 focus:ring focus:outline-none"
          >
            Submit
          </button>
        </form>
